...
class VGG(nn.Module):
    def __init__(self, layers, **kwargs):
        ...
        self.conv = nn.Conv2d(1024, self.num_classes, kernel_size=1, padding=0)
    ...
    def forward(self, images, labels=None):
        ...
        score_map = self.conv(feature_map)
        preds = self.avgpool(score_map).squeeze()
...
def make_layers(config, batch_norm=False, **params):
    ...
    dropout_thresholds = None
    dropout_rates = None

    if 'dropout_params' in params:
        dropout_idx = 0
        dropout_thresholds = params['dropout_params']['drop_thresholds']
        dropout_rates = params['dropout_params']['drop_rates']
...
def make_configs(config, **params):
    ...
    if 'dropout_params' in params:
        dropout_types = params['dropout_params']['types']
        dropout_positions = params['dropout_params']['positions']
    ...
    if dropout_types is not None:
        for dropout_type, dropout_position in zip(dropout_types, dropout_positions):
            dropout_position = str(dropout_position)
            idx = positions.index(dropout_position) + 1
            block = dropout_position[0]
            positions.insert(idx, block + dropout_type)
    ...
    logger.infov('VGG network structure: {}'.format(new_config))
    return new_config
